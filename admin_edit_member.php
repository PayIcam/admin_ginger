<?php
	...
	if (isset ($_POST['login'],$_POST['nom'],$_POST['prenom'])) {
		...
		$validate = array(
			...
			'mail' => array('rule'=>'^[a-z-]+[.]+[a-z-]+([.0-9a-z-]+)?@(mgf\.)?(fp-lille\.)?([0-9]{4}[.])?icam[.]fr$','message' => 'Entrez un email Icam valide !')
	    );
	    ...
	    if ($form->validates($d)) { // fin pré-traitement
	    	...
	    	}elseif (!empty($d['badge_uid']) && current($DB->queryFirst('SELECT COUNT(*) FROM users WHERE login != :login AND badge_uid LIKE :badge_uid',array('login'=>$d['login'],'badge_uid'=>'%'.$d['badge_uid'].'%'))) ) {
